...
and that name opens their own record like every name in a manager's list;
an approval recorded before the club started keeping that (or one whose
approver's account is gone) reads as unknown rather than as nobody, and a waiver
nobody has decided yet has no approver to open. The submissions themselves load with the page; the
