-- Add a media/promotional-photo consent acknowledgement to the waiver.
--
-- The very first waiver template (20260720154139, deleted before launch) had
-- an optional "I may consent to photos and video..." checkbox, but it did not
-- survive the application-form rewrite (20260729010000) or the declaration
-- fix that followed it (20260729150000) — neither carries a media/photo
-- consent item. The club wants it back, worded to cover minors explicitly and
-- to make clear no name is published alongside any photo or video.
--
-- Everything else (body_md, the other acknowledgements) is unchanged from the
-- version this replaces.
--
-- Ships as a NEW version, is_current = false (a draft): a manager reads it and
-- promotes it at /manager/waiver-template. Nothing about the live signing page
