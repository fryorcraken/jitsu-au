// `filePaperWaiver` is the one piece of waiver.functions.ts reachable from a
// unit test without a Start request context: it takes its admin client as a
// parameter rather than lazy-importing it, because it is shared by the
// manager's upload form (a createServerFn handler) and the manager agent HTTP
// API (a plain route handler, authenticated by bearer token instead of a
// Supabase session) — see src/routes/api/manager/agent.ts action `file_waiver`.
//
